...
test('resizeAppliances preserva dados existentes e cria somente os blocos faltantes', () => {
  ...
  assert.equal(expanded[1].id, 'apr-2');
  assert.equal(expanded[1].situacao, 'Não informado');
  assert.equal(expanded[2].id, 'apr-3');
...
test('createEnvironment cria ambiente editável vazio com estrutura estável', () => {
  ...
  assert.equal(ambiente.identificacao, '');
  assert.equal(ambiente.nome_exibicao, 'Sala de Aula');
  assert.equal(ambiente.quantidade_aparelhos, 0);
  assert.deepEqual(ambiente.aparelhos, []);
});

test('normalizeSurvey mantém campos técnicos opcionais e recalcula nomes/quantidades', () => {
  ...
  assert.equal(normalized.ambientes[0].aparelhos[0].modelo_marca, '');
});

test('validateSurvey aceita campos técnicos vazios e rejeita situação fora do catálogo', () => {
  assert.deepEqual(APPLIANCE_STATUS_OPTIONS, [
    'Não informado',
    'Funcionando',
    ...
    ambientes: [
      {
        ...
        tipo: 'Biblioteca',
        identificacao: '',
        nome_exibicao: 'Biblioteca',
        quantidade_aparelhos: 1,
        aparelhos: [
          {
            id: 'apr-1',
            modelo_marca: '',
  ...
  const invalid = validateSurvey({
    ...
    ambientes: [
      {
        ...valid.data.ambientes[0],
        aparelhos: [{ ...valid.data.ambientes[0].aparelhos[0], situacao: 'Crítico' }]
      }
    ]
  });
