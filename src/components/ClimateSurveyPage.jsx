...
import { Building2, Plus, Save, Snowflake, Trash2 } from 'lucide-react';
  ...
  return (
    <section className="climate-survey-appliance" data-appliance-index={index}>
      ...
      <div className="climate-survey-field climate-survey-field--full">
        ...
        <input id={`${prefix}-modelo`} type="text" value={appliance.modelo_marca}
          onChange={(event) => onChange('modelo_marca', event.target.value)}
          placeholder="Digite ou selecione se já conhecer" autoComplete="off" />
      </div>

      <div className="climate-survey-field-grid">
        <div className="climate-survey-field">
          <label htmlFor={`${prefix}-btu`}>Capacidade (BTU)</label>
          <input id={`${prefix}-btu`} type="text" inputMode="numeric" list="climate-survey-btu-options"
            value={appliance.capacidade_btu} onChange={(event) => onChange('capacidade_btu', event.target.value)}
            placeholder="Ex.: 12000" autoComplete="off" />
        </div>
        <div className="climate-survey-field">
          <label htmlFor={`${prefix}-tipo`}>Tipo do aparelho</label>
          <input id={`${prefix}-tipo`} type="text" list="climate-survey-type-options"
            value={appliance.tipo} onChange={(event) => onChange('tipo', event.target.value)}
            placeholder="Ex.: Split" autoComplete="off" />
        </div>
      </div>

      <div className="climate-survey-field climate-survey-field--full">
        <label htmlFor={`${prefix}-situacao`}>Situação</label>
        <select id={`${prefix}-situacao`} value={appliance.situacao}
          onChange={(event) => onChange('situacao', event.target.value)}
          className={appliance.situacao === 'Funcionando' ? 'is-working' : appliance.situacao === 'Precisa de manutenção' ? 'needs-maintenance' : ''}>
          {APPLIANCE_STATUS_OPTIONS.map((option) => <option key={option} value={option}>{option}</option>)}
        </select>
      </div>

      <div className="climate-survey-field climate-survey-field--full">
        <label htmlFor={`${prefix}-observacoes`}>Observações</label>
        <textarea id={`${prefix}-observacoes`} rows="3" value={appliance.observacoes}
          onChange={(event) => onChange('observacoes', event.target.value)} placeholder="Opcional" />
      </div>
    </section>
  );
}
...
export default function ClimateSurveyPage() {
  ...
  const school = useMemo(() => (dbData?.escolas || []).find((candidate) => candidate.designacao === designacao), [designacao]);
  const storageKey = `gop_climate_survey_${designacao}`;
  const suggestions = useMemo(() => getTechnicalSuggestions(), []);
  const [survey, setSurvey] = useState(() => school ? loadLocalSurvey(storageKey, school) : { designacao, unidade_escolar: '', ambientes: [] });
  const [activeEnvironmentId, setActiveEnvironmentId] = useState(() => survey.ambientes[0]?.id || '');
  ...
  const activeEnvironment = survey.ambientes.find((environment) => environment.id === activeEnvironmentId) || survey.ambientes[0];

  const updateEnvironment = (environmentId, updater) => {
    setSurvey((current) => ({ ...current, ambientes: current.ambientes.map((environment) => environment.id === environmentId ? updater(environment) : environment) }));
  };
  ...
  const updateEnvironmentField = (field, value) => {
    ...
    updateEnvironment(activeEnvironment.id, (environment) => {
      ...
      if (field === 'tipo' || field === 'identificacao') next.nome_exibicao = buildDisplayName(next);
      return next;
    });
  };
  ...
  const updateQuantity = (value) => {
    ...
    updateEnvironment(activeEnvironment.id, (environment) => ({ ...environment, quantidade_aparelhos: count, aparelhos: resizeAppliances(environment.aparelhos, count) }));
  };

  const updateAppliance = (applianceId, field, value) => {
    if (!activeEnvironment) return;
    updateEnvironment(activeEnvironment.id, (environment) => ({ ...environment, aparelhos: environment.aparelhos.map((appliance) => appliance.id === applianceId ? { ...appliance, [field]: value } : appliance) }));
  };

  const addEnvironment = () => {
    const environment = createEnvironment();
    setSurvey((current) => ({ ...current, ambientes: [...current.ambientes, environment] }));
  ...
  const saveSurvey = () => {
    ...
    const localRecord = normalizeSurvey({ ...validation.data, atualizado_em: new Date().toISOString() });
    ...
    <main className="climate-survey-page">
      ...
      <div className="climate-survey-shell">
        <header className="climate-survey-header">
          <div className="climate-survey-title-icon" aria-hidden="true"><Snowflake size={28} /></div>
          <div>
            <p className="climate-survey-eyebrow">3ª CRE · GOP</p>
            <h1>Prontuário de Climatização</h1>
            <p>Levantamento por ambientes</p>
          </div>
          <div className="climate-survey-preview-badge">Prévia local</div>
        </header>

        <section className="climate-survey-school" aria-label="Unidade escolar">
          <Building2 size={22} aria-hidden="true" />
          <div><span>Unidade escolar</span><strong>{school.unidade_escolar}</strong></div>
          <small>Designação {school.designacao}</small>
        </section>

        <datalist id="climate-survey-environment-options">{ENVIRONMENT_TYPE_OPTIONS.map((option) => <option key={option} value={option} />)}</datalist>
        <datalist id="climate-survey-btu-options">{suggestions.btu.map((option) => <option key={option} value={option} />)}</datalist>
        <datalist id="climate-survey-type-options">{suggestions.applianceTypes.map((option) => <option key={option} value={option} />)}</datalist>
        ...
        <nav className="climate-survey-environment-nav" aria-label="Ambientes cadastrados">
          <div className="climate-survey-environment-tabs">
            {survey.ambientes.map((environment) => (
              <button type="button" key={environment.id} className={environment.id === activeEnvironment?.id ? 'is-active' : ''}
                onClick={() => setActiveEnvironmentId(environment.id)}>{environment.nome_exibicao || 'Novo ambiente'}</button>
            ))}
          </div>
          <button type="button" className="climate-survey-add-environment" onClick={addEnvironment}><Plus size={17} /> Novo ambiente</button>
        </nav>
          ...
          <section className="climate-survey-form-card">
            <div className="climate-survey-environment-fields">
              <div className="climate-survey-field">
                ...
                <input id="survey-environment-type" list="climate-survey-environment-options" value={activeEnvironment.tipo}
                  onChange={(event) => updateEnvironmentField('tipo', event.target.value)} autoComplete="off" />
              </div>
              <div className="climate-survey-field">
                <label htmlFor="survey-environment-id">Identificação do ambiente</label>
                <input id="survey-environment-id" type="text" value={activeEnvironment.identificacao}
                  onChange={(event) => updateEnvironmentField('identificacao', event.target.value)} placeholder="Ex.: 101, A, Bloco B" autoComplete="off" />
              </div>
              <div className="climate-survey-field climate-survey-quantity-field">
                ...
                <select id="survey-appliance-quantity" value={String(activeEnvironment.quantidade_aparelhos)} onChange={(event) => updateQuantity(event.target.value)}>
            ...
            <div className="climate-survey-current-environment">
              ...
              {survey.ambientes.length > 1 && <button type="button" onClick={removeActiveEnvironment}><Trash2 size={15} /> Excluir ambiente</button>}
            </div>
            ...
            {activeEnvironment.aparelhos.length === 0 ? (
              ...
              <div className="climate-survey-appliance-grid">
                {activeEnvironment.aparelhos.map((appliance, index) => (
                  <ApplianceCard key={appliance.id} environmentId={activeEnvironment.id} appliance={appliance} index={index}
        ...
        <footer className="climate-survey-footer">
          <div className="climate-survey-save-feedback" role="status" aria-live="polite">{saveMessage}</div>
          <button type="button" className="climate-survey-save" onClick={saveSurvey}><Save size={18} /> Salvar rascunho</button>
        </footer>
      </div>
    </main>
  );
