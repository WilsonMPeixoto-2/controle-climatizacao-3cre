...
export const DEFAULT_APPLIANCE_TYPE_OPTIONS = Object.freeze([
  'Split', 'Split Hi Wall', 'Janela', 'Cassete', 'Piso Teto'
]);
...
export function resizeAppliances(appliances = [], targetCount = 0, { idFactory = () => createId('apr') } = {}) {
  ...
  while (resized.length < count) resized.push(createAppliance({ idFactory }));
  return resized;
}
